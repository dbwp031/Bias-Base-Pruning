...
def count_gkconvNd(m, x, y):
    x = x[0]
    global total_ops

    #### case2 ####
    base_size = m.gen.bank.shape[1]
    num_basis = m.gen.bank.shape[0]
    ...
    # # N x Cout x H x W x  (Cin x Kw x Kh + bias)
    conv_ops = mid_size * (m.in_channels//m.groups  * num_basis )  * base_size
    combing_ops = m.map.numel() * mid_size      #Assuming The sparsity can be utilized.
    ...
    # # N x Cout x H x W x  (Cin x Kw x Kh + bias)
    # total_ops = y.nelement() * (m.in_channels // m.kwargs['groups'] * kernel_ops + bias_ops)
    # total_ops += 2 * m.map.size(0) * m.map.size(1) * m.gen.bank.size(1)
    # m.total_ops += torch.DoubleTensor([int(total_ops)])
